...
def find_by_doctor(patients: list[dict]) -> None:
    ...
    names: list[str] = [patient["patient_name"] for patient in patients]

    doctors: list[str] = [patient["doctor"] for patient in patients]

    found: bool = False

    for patient_name, patient_doctor in zip(names, doctors):

        if patient_doctor.lower() == doctor.lower():

            print(patient_name, "-", patient_doctor)
            found = True
...
def sort_by_age(patients: list[dict]) -> None:
    ...
    sorted_patients: list[dict] = sorted(
        patients,
